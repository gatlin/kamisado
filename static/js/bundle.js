...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(5)], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, board_1) {
	    ...
	    var KamisadoAI = (function () {
	        ...
	        KamisadoAI.prototype.getMoves = function (board, whereAmI, player) {
	            var results = [];
	            var dY = player ? 1 : -1;
	            var winY = player ? 7 : 0;
	            for (var dX = -1; dX < 2; dX++) {
	                var pos = whereAmI.clone();
	                var done = false;
	                while (!done) {
	                    pos.x = pos.x + dX;
	                    pos.y = pos.y + dY;
	        ...
	        KamisadoAI.prototype.nextMove = function (board) {
	            ...
	            var myMoves = this.getMoves(board, board.active, this.whoAmI);
	            ...
	            var moveIndex = myMoves
	                .reduce(function (mI, pos) {
	                var color = board_1.tileColorPattern[pos.y][pos.x];
	                var theirPiece = (color + 1) + (otherPlayer * 8);
	                if (!(theirPiece in mI)) {
	                    mI[theirPiece] = [pos];
	                }
	                else {
	            ...
	            // possibilities
	            var loseY = (winY - 7) % 8;
	            for (var pieceS in moveIndex) {
	                var piece = parseInt(pieceS);
	                var pos = this.findPiece(board, piece);
	                var pieceMoves = this.getMoves(board, pos, otherPlayer);
	                var pieceMovesNum = pieceMoves.length;
	                console.log('piece ' + piece + ' moves:');
	                console.log(pieceMoves);
	                for (var _a = 0, pieceMoves_1 = pieceMoves; _a < pieceMoves_1.length; _a++) {
	                    var move = pieceMoves_1[_a];
	                    if (move.y === loseY) {
	                ...
	                if (pieceMovesNum < lowest_option_num) {
	                    ...
	                    var idx = Math.floor(Math.random() * moveIndex[piece].length);
