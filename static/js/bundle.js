...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(1), __webpack_require__(6)], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, game_1, ai_1) {
	    "use strict";
	    var runtime = game_1.kamisadoApp.start();
	    var ai = new ai_1.KamisadoAI(1);
	    // tell the game when to resize
	    window.addEventListener('resize', function (evt) {
	        runtime.ports.inbound.resize.send(evt);
	    }, false);
	    // for game <-> AI interaction
	    runtime.ports.outbound.moves.recv(function (board) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(2), __webpack_require__(5)], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, alm_1, board_1) {
	    ...
	    (function (Actions) {
	        ...
	        Actions[Actions["UserMove"] = 2] = "UserMove";
	        Actions[Actions["Reset"] = 3] = "Reset";
	        Actions[Actions["RemoteMove"] = 4] = "RemoteMove";
	    })(Actions || (Actions = {}));
	    ;
	    ...
	    function load_game() {
	        ...
	        if (!saved) {
	            ...
	            if (parsed) {
	                return new board_1.Board(parsed.grid, new board_1.Pos(0, 0), parsed.gameId, (parsed.active !== null
	                    ? new board_1.Pos(parsed.active.x, parsed.active.y)
	                    : null), parsed.player);
	            }
	            else {
	                erase_game();
	    ...
	    function update(action, state) {
	        if (action['type'] === Actions.ResizeStart) {
	            ...
	            var resizeFinish_1 = function () {
	                if (Date.now() - state.resizeStart < 200) {
	                    ...
	                    state.board = board_1.drawCells(state.board, state.context, state.geom);
	                }
	            };
	        ...
	        if (action['type'] === Actions.UserMove &&
	            ...
	            var src = state.board.active !== null
	            ...
	            var dst = pos.clone();
	            var success = board_1.movePiece(state.board, pos);
	        ...
	        if (action['type'] === Actions.RemoteMove) {
	            var success = board_1.movePiece(state.board, action.data.clone());
	            if (success) {
	                state.current_player = (state.current_player + 1) % 2;
	                if (state.board.won !== null) {
	                }
	    ...
	    function main(scope) {
	        ...
	        scope.state
	            .reduce(0, function (state, move_number) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports) {
	    ...
	    var Pos = (function () {
	        ...
	        Pos.prototype.becomes = function (that) {
	            this.x = JSON.parse(JSON.stringify(that.x));
	            this.y = JSON.parse(JSON.stringify(that.y));
	            return this;
	        };
	        Pos.prototype.clone = function () {
	            return new Pos(JSON.parse(JSON.stringify(this.x)), JSON.parse(JSON.stringify(this.y)));
	        };
	        return Pos;
	    }());
	    exports.Pos = Pos;
	    var Board = (function () {
	        ...
	        Board.prototype.map = function (f) {
	            return new Board(this.grid.map(f), this.pos, this.gameId, this.active, this.player);
	        };
	        ...
	        Board.prototype.convolve = function (f) {
	            ...
	            return new Board(grid.map(f), this.pos, this.gameId, this.active, this.player);
	        };
	        // Determines the next piece based on the color of the cell landed on
	        Board.prototype.selectNextPiece = function () {
	            var x, y, nextCell;
	            var activeColor = exports.tileColorPattern[this.active.y][this.active.x];
	    ...
	    function drawCells(board, context, geom) {
	    ...
	    */
	    ...
	    function drawCell(context, geom) {
	        return function (board) {
	            var tileSide = geom.tileSide / geom.pixelRatio;
	            var radius = geom.radius / geom.pixelRatio;
	            // draw the background color
	            var cell = board.extract(); // `pos`
	            var cellColor = exports.colors[exports.tileColorPattern[board.pos.y][board.pos.x]];
	            context.fillStyle = cellColor;
	            var rectStart = [(board.pos.x * tileSide),
	            ...
	            var bezel = (cell > 8) ? exports.colors[9] : exports.colors[8];
	            var color = exports.colors[(cell - 1) % 8];
	            // bezel
	            context.beginPath();
	    ...
	    function movePiece(board, pos) {
	        var cell = board.setPos(pos).extract();
	        if (board.active === null) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;!(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__, exports, __webpack_require__(5)], __WEBPACK_AMD_DEFINE_RESULT__ = function (require, exports, board_1) {
	    "use strict";
	    var KamisadoAI = (function () {
	        function KamisadoAI(whoAmI) {
	            this.whoAmI = whoAmI;
	        }
	        KamisadoAI.prototype.getMoves = function (board, whereAmI, player, ignore) {
	            if (ignore === void 0) { ignore = null; }
	            var results = [];
	            var stepY = player ? 1 : -1;
	            var winY = player ? 7 : 0;
	            for (var _i = 0, _a = [1, 0, -1]; _i < _a.length; _i++) {
	                var stepX = _a[_i];
	                var dY = stepY, dX = stepX, pos = void 0;
	                var done = false;
	                while (!done) {
	                    pos = whereAmI.clone();
	                    pos.x += dX;
	                    pos.y += dY;
	                    var ignorePos = (ignore !== null && pos.equals(ignore));
	                    if (pos.y > 7 || pos.y < 0 ||
	                        pos.x > 7 || pos.x < 0 ||
	        ...
	        KamisadoAI.prototype.findPiece = function (board, piece) {
	            var x, y, nextCell;
	            for (y = 0; y < 8; y++) {
	                for (x = 0; x < 8; x++) {
	                    nextCell = board.gridGet(x, y);
	                    if (nextCell === piece) {
	                        return new board_1.Pos(x, y);
	                    }
	                }
	            }
	            return null;
	        ...
	        KamisadoAI.prototype.nextMove = function (board) {
	            if (board.player !== this.whoAmI) {
	                return null;
	            }
	            var otherPlayer = (this.whoAmI + 1) % 2;
	            var winY = this.whoAmI ? 7 : 0;
	            var loseY = this.whoAmI ? 0 : 7;
	            var myMoves = this.getMoves(board, board.active, this.whoAmI, null);
	            ...
	            var lowest_scoring_piece = null;
	            if (myMoves.length === 0) {
	                return board.active;
	            }
	            for (var _i = 0, myMoves_1 = myMoves; _i < myMoves_1.length; _i++) {
	                var move = myMoves_1[_i];
	                if (move.y === winY) {
	                    return move;
	                }
	                var color = board_1.tileColorPattern[move.y][move.x];
	                var piece = (color + 1) + (otherPlayer * 8);
	                ...
	                var piece_pos = this.findPiece(board, piece);
	                var piece_moves = this.getMoves(board, piece_pos, otherPlayer, board.active);
	                opponent_moves[piece] = piece_moves;
	                var penalty = 0;
	                // if the piece can win, exclude it
	                for (var _a = 0, piece_moves_1 = piece_moves; _a < piece_moves_1.length; _a++) {
	                    var piece_move = piece_moves_1[_a];
	                    if (piece_move.y === loseY) {
	                        penalty += 25;
	                    }
	                }
	                var score = penalty + piece_moves.length;
	                if (score < lowest_score) {
	                    lowest_score = score;
	                    lowest_scoring_piece = piece;
	                }
	            }
	            var max = opponent_pieces[lowest_scoring_piece].length;
	            return opponent_pieces[lowest_scoring_piece][Math.floor(Math.random() * max)];
	        };
	        return KamisadoAI;
	    }());
	    exports.KamisadoAI = KamisadoAI;
	}.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__), __WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
